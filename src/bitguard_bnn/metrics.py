...
def calibrate_fixed_fpr_thresholds(
    ...
    if probabilities.ndim != 2 or probabilities.shape[0] != y_true.shape[0]:
        raise ValueError(
            "probabilities must be a two-dimensional array aligned with y_true"
        )
    ...
    for value in target_fprs:
        ...
        if allowed_false_positives == 0:
            ...
            if (
                int(np.count_nonzero(benign_attack_scores >= threshold))
...
def classification_metrics(
    ...
    metrics["unknown_like_recall"] = (
        float(np.mean(y_pred[unknown_mask] == "unknown_like"))
        if unknown_mask.any()
        else None
    )
    ...
    for column_index, label in enumerate(probability_labels):
        ...
        try:
            ...
            auprc[label] = float(
                average_precision_score(target, probabilities[:, column_index])
            )
        except ValueError:
            continue
    ...
    true_columns = np.asarray(
        [label_to_column[label] for label in y_true], dtype=np.int64
    )
...
def confusion_frame(y_true: np.ndarray, y_pred: np.ndarray) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        matrix,
        index=[f"true_{item}" for item in labels],
        columns=[f"pred_{item}" for item in labels],
    )
...
def make_plots(
    ...
    for label in probability_labels:
        ...
        PrecisionRecallDisplay.from_predictions(
            target, probability, name=label, ax=axis_pr
        )
...
def estimate_dense_operations(
    input_dim: int, hidden_dims: list[int], output_dim: int
) -> int:
    dimensions = [input_dim, *hidden_dims, output_dim]
    return int(
        sum(left * right for left, right in zip(dimensions[:-1], dimensions[1:]))
    )
